function s=splinecubico(x,y,zi)
% SPLINECUBICO Spline Cubico
% s=splinecubico(x,y,zi) calcula o spline cubico, S, associado a (x(i),y(i)), i=1,...,length(x),
% no ponto zi, ie, s=S(zi).
[n,m]=size(x);
if n==1
    x=x';  y=y';  n=m;
end
...
if type==0
    ...
    rhs=[3*(der(1)-der0); rhs; 3*(dern-d(end))];
else
    A(1,:)=0; A(1,1)=1;
    A(n,:)=0; A(n,n)=1;
    rhs=[der0;rhs;dern];
