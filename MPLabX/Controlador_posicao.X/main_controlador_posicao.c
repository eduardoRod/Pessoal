 ...
 * 
 *                 Encoder      7474        Notas
 * pin26 (TCKIA)   Amarelo      pin2 (D)    Colocar pull-up 1-10k
 *                 Azul         pin3 (CLK)  Colocar pull-up 1-10k
 * pin10 (RA3)                  pin5 (Q)    Direccao
 */
...
#define ADCToStepRatio 1.4 // max_Theta = 5750, max_ADC = 4095
...
// Controlador
int16_t SpeedError, PositionError;
...
int main(void) {
    ...
    printf("\rControlador PID de posicao\r\n");
    ...
    while (1) {
        if (control_flag) {
            ...
            dError = SpeedError;
            int16_t ref = (readADC(10)*ADCToStepRatio) - (2046*ADCToStepRatio);
            PositionError = ref - encX.Theta;
            SpeedError = PositionError - encX.dTheta;
...
inline void ConfigIO(void) {
    ANSA = 0;
    TRISAbits.TRISA1 = 0; // Dir      (RA0)
    //ANSAbits.ANSA1 = 1;
    //TRISAbits.TRISA1 = 1; // Potenciometro
    //TRISAbits.TRISA3 = 1; // DirEncoder 
